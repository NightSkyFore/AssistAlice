...
class AliceAI:
    ...
    def get_response(self, content: str) -> Union[
        CreateChatCompletionResponse, Iterator[CreateChatCompletionStreamResponse]
    ]:
        self.dialog_manager.add("user", content)
        messages = [
            self.system_message,
            *self.dialog_manager.build()
        ]
        tool = custom_tools.tool_routing(content)
        tool_res = None
        if callable(tool):
            tool_res = tool()
        ...
        if res["choices"][0]["message"]["content"]:
            self.dialog_manager.add(res["choices"][0]["message"]["role"], res["choices"][0]["message"]["content"])

            if self.dialog_manager.needSummurize():
                history_content = self.dialog_manager.buildToSummarize()
                history_messages = [{
                    "role": "system",
                ...
                if res["choices"][0]["message"]["content"]:
                    self.dialog_manager.updateSummry(res["choices"][0]["message"]["content"])
        return res
...
if __name__ == "__main__":
    alice = AliceAI()
    alice.get_response("extract John with age 24.")
    alice.get_response("google the latest news.")
    alice.get_response("what's the time now?")
    alice.get_response("What's the height of TaiShan?")
